...
#show: resume.with(
  ...
  本科期间专注于计算机体系结构与系统软件方向，独立完成编译器后端、CPU 设计、操作系统内核等多个系统级项目或实验。
  曾获华为毕昇杯编译系统设计赛三等奖；目前在"一生一芯"线下基地参与 RISC-V 处理器设计。对处理器微架构、操作系统、编译优化有浓厚兴趣，希望在研究生阶段继续深耕体系结构领域。
...
- *主动学习*：在课程体系基础上积极拓展，将课程实验升级为更具挑战性的开源项目：xv6 → Pintos; 计网实验 → CS144; minirv → 一生一芯; 编译原理实验 → 毕昇杯。
- *技术热情*：主动探索前沿工具链，使用 Chisel 进行硬件开发、Rust 构建系统软件、Nix 管理项目依赖与构建环境。
- *乐于助人*：担任班长四年，参与院学生会权益服务工作。
